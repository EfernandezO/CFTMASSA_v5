...
if($_POST){
	...
	{ $condicion_year_cuota="AND letras.ano='$year_cuotas'";}
		...
		if($totalAlumnos>0)
		{
			...
			//////------------------------/INICIO PDF/------------------//////////
			$html_inicial=utf8_decode('Sr(ita) <b>'.$nombre.' '.$apellidos.'</b><br>Carrera: '.NOMBRE_CARRERA($id_carrera_alumno).'<br>'.$direccion.'<br>'.$ciudad.'<br>');
			$html_texto_1='Estimado(a) '.utf8_decode($nombre).' '.utf8_decode($apellidos);
			$html_texto_1b='Junto con saludar, nos ponemos en contacto con Ud. por las mensualidades establecidas en su contrato de '.utf8_decode("prestación").' de servicios con CFT Massachusetts.';
			
			
			
			$html_texto_3=utf8_decode('El no cumplimiento de los compromisos adquiridos en la fecha señalada, nos faculta para realizar las cobranzas notarial por el monto total del pagaré');
			...
			//Primera página
					...
					$html_texto_2=utf8_decode('El Motivo de la comunicacion es que hasta la fecha no hemos recibido el pago de las cuotas pendientes ['.$num_cuotas.' Cuota(s)], por lo que le solicitamos cancelar antes del ').$fecha_limite;
