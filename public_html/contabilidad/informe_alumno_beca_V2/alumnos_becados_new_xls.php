...
<tbody>';
	...
	require("../../../funciones/conexion_v2.php");	
		...
		{ 
			if($hay_condiciones)
			{$condicion_ingreso="AND contratos2.yearIngresoCarrera='$year_ingreso'";}
			else
			{ $condicion_ingreso=" contratos2.yearIngresoCarrera='$year_ingreso'";}
			$hay_condiciones=true;
		}
		else
		{ $condicion_ingreso="";}
		...
		{
			if($hay_condiciones)
			{ $condicion_carrera="AND contratos2.id_carrera='$id_carrera'";}
			else
			{ $condicion_carrera="contratos2.id_carrera='$id_carrera'";}
			$hay_condiciones=true;
		}
		else
		{ $condicion_carrera="";}
		...
		$sql_main_1=$conexion_mysqli->query($cons_main_1);
		...
		if($num_reg_M>0)
		{
			...
			while($DID=$sql_main_1->fetch_row())
			{
				...
				$cons_main_2="SELECT MAX(id) FROM contratos2 WHERE id_alumno='$id_alumno' AND ano='$contrato_year' AND condicion<>'inactivo' AND (aporte_beca_nuevo_milenio<>0 OR aporte_beca_excelencia<>0 OR cantidad_beca<>0 OR porcentaje_beca<>0)";
				if(DEBUG){ echo"<br>>$cons_main_2<br>";}
				$sql_main_2=$conexion_mysqli->query($cons_main_2);
				...
				$sql_main_2->free();	
					...
					$sql_main=$conexion_mysqli->query($cons_main);
					...
					if($num_registros>0)
					{
						...
						while($DB=$sql_main->fetch_assoc())
						{
							...
							if(DEBUG){ echo"id_alumno: $A_id <br>Rut: $A_rut<br>id_contrato: $C_id Semestre Contrato: $C_semestre Year Contrato: $C_ano BNM: $C_beca_nuevo_milenio Aporte BNM: $C_aporte_beca_nuevo_milenio<br>info: $C_txt_beca<br><br>";}
								...
								{ $tiene_BNM=true; if(DEBUG){ echo"<strong>Tiene BNM</strong><br>";} if($C_aporte_beca_nuevo_milenio>0){$color_1="#00FF00";}else{$color_1="#AAFF00";}}
								else
							...
							if($mostrar_alumno and $contrato_OK)
							{
								...
								<td align="center" bgcolor="'.$color_4.'">'.$C_p_otros_desc.'</td>
								<td align="center" bgcolor="'.$color_4.'">'.$totalizado_desc.'</td>
								<td>'.$C_num_cuotas.'</td>
...
<tr bgcolor="#CCFF99">
	<td colspan="8"><strong>Cantidad Alumnos Con Beca Nuevo Milenio </strong></td>
...
<tr bgcolor="#CCFF99">
	<td colspan="8"><strong>Cantidad Alumnos Con Beca Excelencia Tecnica</strong></td>
...
<tr bgcolor="#CCFF99">
	<td colspan="8"><strong>Cantidad Alumnos Con Desc. cantidad</strong></td>
...
<tr bgcolor="#CCFF99">
	 <td colspan="8"><strong>Cantidad Alumnos Con Desc. porcentaje</strong></td>
