...
//--------------CLASS_okalis------------------//
	...
	$O->clave_del_archivo=md5("informe_pagos_especificos_X_Rango_v1");
...
<title>Detalle de pagos especificos</title>
...
if($_POST)
{
	...
	$A=busca_pagos($fecha_inicio, $fecha_fin, $sede, $por_concepto);
	...
	$evento="Revisa Informe Pagos Especificos X Rango de Fechas periodo[$fecha_inicio - $fecha_fin] sede: $sede movimiento $movimiento";
...
//////////////////////////////
function busca_pagos($fecha_inicio, $fecha_fin, $sede, $por_concepto)
{
	...
	$img_ok='<img src="../../BAses/Images/ok.png" width="10" height="10"/>';
		...
		$condicion1="AND pagos.por_concepto='$por_concepto'";
		
		if($sede=="todas"){ $condicion_sede="";}
		else{ $condicion_sede=" AND pagos.sede='$sede'";}
		
		
		$consP="SELECT pagos.*, alumno.rut, alumno.nombre, alumno.apellido_P, alumno.apellido_M, alumno.carrera FROM pagos INNER JOIN alumno ON pagos.id_alumno=alumno.id WHERE pagos.fechapago BETWEEN '$fecha_inicio' AND '$fecha_fin' $condicion_sede $condicion1 ORDER by fechapago, idpago";
		if(DEBUG){ echo "<br><strong>$consP</strong><br>";}
		...
		else
		{
			...
			$html_tabla.='<tr>
					<td colspan="12">Sin pagos Registrados en este Rango de Fechas, sede y concepto</td>
					</tr>';
		}
		$html_tabla.='</tbody>
				<tfoot>
