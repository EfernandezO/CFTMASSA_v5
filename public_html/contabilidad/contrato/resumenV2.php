...
// $año_contrato=end(explode("-",$_SESSION["FINANZAS"]["fecha_inicio"]));
 ...
 //------------------------------------------------------
//verificar que no tengo otro contrato para el periodo

$repetido_contrato=false;
$msj_error="";

 $consA="SELECT COUNT(id) FROM contratos2 WHERE id_alumno='$id_alumno' AND sede='$sede_alumno' AND ano='$año_contrato' AND vigencia='anual' AND condicion<>'INACTIVO'";
 if(DEBUG){echo "CONTRATO ANUAL $consA<br>";}
 $sqlC=$conexion_mysqli->query($consA)or die($conexion_mysqli->error);
 $D=$sqlC->fetch_row();
 $n_regs_contrato=$D[0];
 if(DEBUG){echo "n. contratos: $n_regs_contrato<br>";}
 if($n_regs_contrato>0)
 {
 	$msj_error="El Alumno ya tiene un contrato de vigencia anual generado en este año, sede y carrera";
	$repetido_contrato=true;
 }
 else
 {
		$consS="SELECT COUNT(id) FROM contratos2 WHERE id_alumno='$id_alumno' AND sede='$sede_alumno' AND ano='$año_contrato' AND semestre='$semestre' AND vigencia='semestral' AND condicion<>'INACTIVO'";
	 if(DEBUG){echo "SEMESTRAL: $consS<br>";}
	 $sqlS=$conexion_mysqli->query($consS)or die($conexion_mysqli->error);
	 $D=$sqlS->fetch_row();
	 $n_regs_contrato=$D[0];
	 if(DEBUG){echo "n. contratos: $n_regs_contrato<br>";}
	 if($n_regs_contrato>0)
	 {
		$msj_error="El Alumno ya tiene un contrato de vigencia SEMESTRAL generado en este semestre, año, sede y carrera";
		$repetido_contrato=true;
	 }
	$sqlS->free();
 }
...
<div id="Layer1">
  <table width="100%" border="0" align="center">
    ...
    <tr>
      <td>Alumno </td>
      <td colspan="3"><em><?php echo $_SESSION["SELECTOR_ALUMNO"]["nombre"]." ".$_SESSION["SELECTOR_ALUMNO"]["apellido"];?></em></td>
...
&nbsp;	  </td>
    ...
    <tr>
      <td colspan="7" bgcolor="#e5e5e5"><strong>Beneficios Estudiantiles</strong></td>
    </tr>
    <?php
	...
	if(isset($_SESSION["FINANZAS"]["beneficiosEstudiantiles"])){
     foreach($_SESSION["FINANZAS"]["beneficiosEstudiantiles"] as $auxIdBeneficio =>$arrayValores){
		 ...
		 echo'<tr>
		 	 <td>-</td>
			 <td>'.$auxNombre.'</td>
			 <td align="right">$'.number_format($totalizadoBeneficio,0,",",".").'</td>
		 </tr>';
	 }
	}
	...
	<td>-</td>
      ...
      <td align="right">
	  <?php 
	  echo "$".number_format($_SESSION["FINANZAS"]["excedente"],0,",",".");
	  ?>
	  </td>
      <td>
      <?php
    ...
    <tr>
      <td height="15">-</td>
      <td>Arancel</td>
      <td align="right"><strong><?php echo $arancel;?></strong></td>
    </tr>
    <tr>
      <td height="15">=</td>
      <td>Total a Pagar</td>
      <td align="right">$
      <?php echo number_format($_SESSION["FINANZAS"]["total_a_pagar_arancel"],0,",",".");?>
      </td>
    </tr>
    <tr>
	...
	<tr>
      ...
      <td colspan="2"><a href="paso2c.php" class="button">Paso 2 </a></td>
      <td><a href="paso_3c.php" class="button">Paso 3 </a></td>
	</tr>
	<tr>
