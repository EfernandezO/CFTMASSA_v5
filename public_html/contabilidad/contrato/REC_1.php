...
{ $hay_alumno_seleccionado=false;}
	...
	if(($iniciar_proceso_grabacion)and($hay_alumno_seleccionado))
	{
		...
		$aux_total=((($arancelV2-$cantidad_beca)-$aux_excedente)-$aporte_beca_nuevo_milenio)-$aporte_beca_excelencia_academica;//resto cantidad
		...
		{$descuentoXbeca=(($porcentaje_beca*$arancelV2)/100);}
		else
		{$descuentoXbeca=0;}
		///////////////////////----/////////////////////////////
		$arancel_total=($aux_total-$descuentoXbeca);
 ...
 //------------------------------------------------------------------------------------//
		...
		$contrato["cantidad_beca"]=$cantidad_beca;/////cantidad_beca
		$contrato["porcentaje_beca"]=$porcentaje_beca;//%
		$contrato["beca_nuevo_milenio"]=$beca_nuevo_milenio;
		$contrato["aporte_beca_nuevo_milenio"]=$aporte_beca_nuevo_milenio;
		$contrato["beca_excelencia_academica"]=$beca_excelencia_academica;
		$contrato["aporte_beca_excelencia_academica"]=$aporte_beca_excelencia_academica;
		...
		if(isset($_SESSION["FINANZAS"]["comentario_beca"])){$contrato["txt_beca"]=$_SESSION["FINANZAS"]["comentario_beca"];}
		else{ $contrato["txt_beca"]="";}
	...
	//	var_export($contrato);
		...
		if(!$continuar)
		{
			...
			//----------------------------------------------------------//
				...
				{ $TOMA_RAMO_NIVEL_1=false;}
				///////////////////////////////////////////
				if(isset($_SESSION["SELECTOR_ALUMNO"]["ingreso"]))
				{
					$ingreso_alumno=$_SESSION["SELECTOR_ALUMNO"]["ingreso"];
				...
				{ if(DEBUG){ echo"NO hacer toma ramo nivel 1<br>";}}
				/////---------------------------------------------------------///
				//--------------------------------------------------------------//
...
function GRABA_CONTRATO($contrato)
{
	...
	$porcentaje_becaX=$contrato["porcentaje_beca"];
	...
	$contrato["txt_beca"]=$comentario_beca_v2." [".$contrato["txt_beca"]."]";
	//------------------------------------------------------------------//
...
function GRABAR_PAG_MAT($datos, $boletas)
{
	...
	{
		if(mysql_query($consIN)){$error=true;}
		else
		{
			$error=false;
			echo "ERROR EN GRABAR_PAG_MAT <br>".mysql_error();
		}
	}
...
function GENERAR_CUOTAS($cuotas)
{
	...
	for($c=1;$c<=$numero_cuotas;$c++)
	{
		...
		{mysql_query($cons)or die("GENERA CUOTA ".mysql_error());}	
...
function GENERA_PAGO($pago)
{
	...
	{mysql_query($consIN)or die("GENERA_PAGO ".mysql_error());}	
	if(DEBUG){ echo"<br><strong>____________________________________INICIO FUNCION GENERA_PAGO__________________________________________</strong><br>";}
}
//-------------------------------------------//
function GRABA_DATO_ALUMNO($id_alumno)
{
	...
	$cons_UP="UPDATE alumno SET id_carrera='$id_carrera', carrera='$carrera', grupo='$grupo', nivel='$nivel', situacion='V' WHERE id='$id_alumno' LIMIT 1";
	if(DEBUG){ echo"<br><strong>Graba datos Alumno -></strong><br> $cons_UP<br>";}
	else
	{mysql_query($cons_UP)or die("GRABA dato alumno ".mysql_error());}
...
function CAMBIAR_CONDICION_CONTRATOS_OLD($id_contrato, $id_alumno, $nueva_condicion)
{
	...
	{mysql_query($cons_upc)or die("condicion contratos old".mysql_error());}
	...
	if($nivel_alumno==1)
	{
		...
		////veo si hay registro previos
		 ...
		 if($grabar_toma_ramos)
		 {
			 ...
			 $sql_BR=mysql_query($cons_BR)or die(mysql_error());
			 $num_ramos=mysql_num_rows($sql_BR);
			 ...
			 if($num_ramos>0)
			 {
				 while($R=mysql_fetch_assoc($sql_BR))
				 {
					 ...
					 $valores="'$id_alumno', '$id_carrera', '$yearIngresoCarrera', '$jornada_alumno', '$nivel_alumno', '$semestre', '$year', '$cod_asignatura', '$condicion', '$fecha_actual', '$id_usuario_robot'";
							...
							else{ mysql_query($cons_IN)or die("$cons_IN <br>TOMA_RAMO_OBLIGATORIA:".mysql_error());}
				 }
