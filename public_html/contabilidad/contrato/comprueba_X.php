	...
	{if($_SESSION["SELECTOR_ALUMNO"]["ACTIVO"]){ $activo=true;}}
	  ...
	  if($activo)
	  {
		 ...
		 $_SESSION["FINANZAS"]["paso1"]=false;
			...
			if($TOTAL_DEUDA_ACTUAL>0){ $js.='\n -- Alumno Con Deuda Actual de : $'.number_format($TOTAL_DEUDA_ACTUAL, 0).' \n\n'; $hay_advertencias=true;}
			
			//comprueba toma de ramos
			/*
			list($TR_semestre, $TR_year)=PERIODO_TOMA_RAMO($id_alumno, $id_carrera, $yearIngresoCarrera, "max");
			
			echo"toma ramo max actual: $TR_semestre $TR_year<br>";
		   ...
		   if($nivel_alumno>1)
		   {
			   ...
			   $periodo_encuesta_year=2020;
			   
			   $conse="SELECT COUNT(id_resultados) FROM encuestas_resultados WHERE id_encuesta='$id_encuesta_JEFE_CARRERA' AND id_usuario='$id_alumno' AND tipo_usuario='alumno' AND semestre_evaluar='$periodo_encuesta_semestre' AND year_evaluar='$periodo_encuesta_year' AND sede_evaluar='$sede_alumno' AND id_carrera_evaluar='$id_carrera'";
		   ...
		   //ENCUESTA EVALUCION DOCENTE
			   ...
			   $cons_ED="SELECT COUNT(DISTINCT(id_usuario_evaluar)) FROM encuestas_resultados WHERE semestre_evaluar='$periodo_encuesta_semestre' AND year_evaluar='$periodo_encuesta_year' AND id_usuario='$id_alumno' AND tipo_usuario='alumno' AND id_encuesta='$id_encuesta_EVALUCION_DOCENTE'";
