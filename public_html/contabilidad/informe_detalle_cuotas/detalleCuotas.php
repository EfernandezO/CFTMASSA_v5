...
<br />
<a href="detalleCuotasXls.php?id_carrera=<?php echo base64_encode($id_carrera);?>&amp;sede=<?php echo base64_encode($sede);?>&amp;year_cuotas=<?php echo base64_encode($year_cuotas);?>" class="button">.XLS </a></div>
<div id="apDiv1">
<?php
if($_POST)
{
...
?>
<strong>Carrera:</strong> <?php echo $id_carrera;?> <strong>Sede:</strong> <?php echo $sede;?><br />
<strong>Año:</strong> <?php echo $year_cuotas;?><br />
...
<?php
		...
		if($num_cuotas_encontradas>0)
		{
			...
			while($L=$sql_CUO->fetch_assoc())	
			{
				...
				$url_destino='../../buscador_alumno_BETA/enrutador.php?validador='.$validador.'&id_alumno='.$A_id;
				
				echo'<tr>
					<td>'.$A_sede.'</td>
					<td><a href="'.$url_destino.'" target="_blank" title="Revisar este Alumno">'.$A_id.'</a></td>
					<td>'.$C_id_carrera.'</td>
					<td>'.$C_jornada.'</td>
...
<?php echo "Generado el ".date("d-m-Y H:i:s");?>
</div>
</body>
</html>