...
function chatHeartbeat() {
	...
	$sql = "select * from chat_x_face where (chat_x_face.to = '".mysqli_real_escape_string($conexion_mysqli, $_SESSION["CHAT"]['nick'])."' AND recd = 0) order by id ASC";
...
EOD;
	...
	$sql = "update chat_x_face set recd = 1 where chat_x_face.to = '".mysqli_real_escape_string($conexion_mysqli, $_SESSION["CHAT"]['nick'])."' and recd = 0";
...
EOD;
	...
	$sql = "insert into chat_x_face (chat_x_face.from,chat_x_face.to,message,sent) values ('".mysql_real_escape_string($from)."', '".mysql_real_escape_string($to)."','".mysql_real_escape_string($message)."',NOW())";
	$query = $conexion_mysqli->query($sql)or die("INSERTANDO ".mysql_error());
	$conexion_mysqli->close();
	echo "1";
	exit(0);
}
