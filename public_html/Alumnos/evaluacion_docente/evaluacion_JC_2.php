<?php	
  ...
  if($continuar and $continuar_2)
  {
	 // list($nombre_asignatura, $nivel_asignatura)=NOMBRE_ASIGNACION($id_carrera, $cod_asignatura);
	  $msj_encuesta="Docente: ".NOMBRE_PERSONAL($id_docente)."<br>Carrera: ".NOMBRE_CARRERA($id_carrera);
	   ...
	   if($num_preguntas>0)
	   {
		...
		//-----------------------------------------//	
		 ...
		 CAMBIA_ESTADO_CONEXION_ALUMNO($id_alumno, "ON");
		  $evento="Revisa Encuesta Evaluacion Docente id_docente: $id_docente id_carrera: $id_carrera";
...
<body>
<h1 id="banner">Evaluación Jefe de Carrera</h1>
<div id="apDiv1">
<form action="<?php echo $action;?>" method="post" id="frm">
<input name="id_encuesta" type="hidden" value="<?php echo $id_encuesta;?>" />
<input name="id_usuario" type="hidden" value="<?php echo $id_alumno; ?>" />
<input name="tipo_usuario" type="hidden" value="<?php echo "alumno";?>" />
