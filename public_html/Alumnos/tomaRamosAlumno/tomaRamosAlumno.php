...
//--------------FIN CLASS_okalis---------------//
   //////////////////////XAJAX/////////////////
	...
	$xajax = new xajax("tomaRamosAlumno_server.php");
...
$num_solicitudes_no_autorizadas=0;
   ...
   $periodoTomaRamos="";
	   for($y=$yearIngresoCarrera;$y<=$year_actual;$y++){
		    for($s=1;$s<=2;$s++){
		   $mostrar=true;
		   //verifico si tiene toma de ramos previa en este periodo
			$cons_TR="SELECT COUNT(id) FROM toma_ramos WHERE id_alumno='$id_alumno' AND semestre='$s' AND year='$y'";
			...
			$sqli_TR->free();
			//---------------------------------//	 
		   if($existe_toma_de_ramos_previa){ $clase_boton='btn btn-large btn-primary'; $funcion='REDIRIGIR(\''.base64_encode($s).'\', \''.base64_encode($y).'\', \''.base64_encode($id_alumno).'\', \''.base64_encode($id_carrera).'\', \''.base64_encode($yearIngresoCarrera).'\')';}
		   else{ $clase_boton='btn btn-large btn-success'; $funcion='xajax_VER_MALLA('.$s.', '.$y.', '.$id_alumno.', '.$id_carrera.', '.$yearIngresoCarrera.'); return false;';}
		    
		   if(($y==$year_actual)and($s>$semestre_actual)and(!$existe_toma_de_ramos_previa)){$mostrar=false;}
		   
		   if($mostrar){$periodoTomaRamos.='<button type="button" class="'.$clase_boton.'" onClick="'.$funcion.'">'.$s.' Semestre -'.$y.'</button>  ';}
...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
   <meta charset="utf-8">
   <title>Toma Ramos</title>
   <?php $xajax->printJavascript(); ?> 
   <script language="javascript">
function CONFIRMAR()
{
	
	c=confirm('Confirma que los Ramos inscritos son los correctos..?\n Si es Asi presione ACEPTAR para  \n Continuar con toma de Ramos...');
	if(c)
	{
...
function REDIRIGIR(s, y, id_alumno, id_carrera, yearIngresoCarrera){
	
	url="../../asignaturas_ramo/ver_toma_ramo/ver_tomaramo_individual.php?id_alumno="+id_alumno+"&id_carrera="+id_carrera+"&yearIngresoCarrera="+yearIngresoCarrera+"&s="+s+"&y="+y
                ...
                <div class="metro-nav">
                    ...
                    <div class="metro-nav-block nav-block-grey" >
                     ...
                     <div class="metro-nav-block nav-block-yellow double">
                        <a data-original-title="Notas P" href="tomaRamosAlumno.php">
                            <i class="icon-edit-sign"></i>
               ...
               <div class="span12">
                  ...
                  <div class="widget red">
                        <div class="widget-title">
                           <h4><i class="icon-reorder"></i> Periodos de Toma de Ramos</h4>
                           <span class="tools">
                        ...
                        <div class="widget-body">
                            <p>
                            <div class="alert alert-info" role="alert">*Seleccione el periodo, para revisar la toma de ramos existente o crear una nueva.<br></div>
                                <?php echo $periodoTomaRamos;?>
                            </p>
                        </div>
                  </div>
                  <!-- END BUTTON PORTLET-->
