...
//-----------------------------------------//	
   ...
   $row = $sqli->fetch_assoc();
		...
		$aplicar_intereses=$row["aplicar_intereses"];
		$aplicar_gastos_cobranza=$row["aplicar_gastos_cobranza"];
	$sqli->free();
	$msj_AVISOS="";
	///////////////////
            ...
            <div class="row-fluid">
                ...
                <div class="metro-nav">
                    ...
                    <div class="metro-nav-block nav-light-purple double">
                        <a data-original-title="Horario" href="revisa_horario.php">
              ...
              <div class="widget purple">
                            <div class="widget-title">
                                <h4><i class="icon-reorder"></i> Horario de Clases <?php echo"[$semestre_actual Semestre - $year_actual]";?></h4>
                            <span class="tools">
                            ...
                            <div class="widget-body">
                                <table class="table table-striped table-bordered table-advance table-hover">
                                    <thead>
                                    <tr>
                                        <th><i class="icon-calendar"></i> Dia</th>
                                        <th><i class="icon-time"></i> Inicio</th>
                                        <th><i class="icon-time"></i> Fin</th>
                                        <th><i class="icon-edit"></i> Asignatura</th>
                                        <th><i class="icon-user"></i> Docente</th>
                                        <th><i class="icon-building"></i> sala</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    <?php
									...
									require("../../../funciones/funciones_sistema.php");
                                     $cons_TR="SELECT * FROM toma_ramos WHERE id_alumno='$id_alumno' AND id_carrera='$id_carrera' AND semestre='$semestre_actual' AND year='$year_actual'";
									 $sqli_tr=$conexion_mysqli->query($cons_TR)or die($conexion_mysqli->error);
									 $num_ramos=$sqli_tr->num_rows;
									 if(DEBUG){ echo"$cons_TR <br> Num Ramos: $num_ramos<br>";}
									 if($num_ramos>0)
									 {
										 while($TR=$sqli_tr->fetch_assoc())
										 {
											 $TR_jornada=$TR["jornada"];
											 $TR_nivel=$TR["nivel"];
											 $TR_cod_asignatura=$TR["cod_asignatura"];
											 
											 if(DEBUG){ echo"jornada: $TR_jornada Nivel: $TR_nivel cod_asignatura: $TR_cod_asignatura<br>";}
											 
											 $cons_asignaciones="SELECT * FROM toma_ramo_docente WHERE sede='$sede' AND id_carrera='$id_carrera' AND cod_asignatura='$TR_cod_asignatura' AND jornada='$TR_jornada' AND semestre='$semestre_actual' AND year='$year_actual'";
											 $sqli_asignaciones=$conexion_mysqli->query($cons_asignaciones)or die($conexion_mysqli->error);
											 $num_asignaciones=$sqli_asignaciones->num_rows;
											 if(DEBUG){ echo"-->$cons_asignaciones<br>-->Num Asignaciones: $num_asignaciones<br>";}
											 if($num_asignaciones>0)
											 {
												 while($AS=$sqli_asignaciones->fetch_assoc())
												 {
													 $AS_id=$AS["id"];
													 $AS_id_funcionario=$AS["id_funcionario"];
													 $AS_grupo=$AS["grupo"];
													 
													 if(DEBUG){ echo"-->id_asignacion: $AS_id id_funcionario: $AS_id_funcionario grupo: $AS_grupo<br>";}
													 
													 $cons_horario="SELECT * FROM horario_docente WHERE id_asignacion='$AS_id'";
													 $sqli_horario=$conexion_mysqli->query($cons_horario)or die($conexion_mysqli->error);
													 $num_horario=$sqli_horario->num_rows;
													 if(DEBUG){ echo"---->$cons_horario<br>---->Num Horario: $num_horario<br>";}
													 if($num_horario>0)
													 {
														 while($H=$sqli_horario->fetch_assoc())
														 {
															 $H_id=$H["id_horario"];
															 $H_dia_semana=$H["dia_semana"];
															 $H_hora_inicio=$H["hora_inicio"];
															 $H_hora_inicio_time=strtotime($H_hora_inicio);
															 $H_hora_fin=$H["hora_fin"];
															 $H_sala=$H["sala"];
															 if(DEBUG){ echo"---->id_horario: $H_id dia_semana: $H_dia_semana Hora inicio: $H_hora_inicio Hora fin: $H_hora_fin<br>";}
															 //lenado de datos
															 $array_datos=array();
															 $array_datos["hora_inicio"]=$H_hora_inicio;
															 $array_datos["hora_fin"]=$H_hora_fin;
															 $array_datos["sala"]=$H_sala;
															 $array_datos["cod_asignatura"]=$TR_cod_asignatura;
															 $array_datos["docente"]=$AS_id_funcionario;
															 
															$ARRAY_HORARIO[$H_dia_semana][$H_hora_inicio_time]=$array_datos;
														 }
													 }
													 else
													 {echo'<tr><td colspan="6">Horario pendiente, Revisar Despues</td></tr>';}
													 $sqli_horario->free();
												 }
											 }
											 else
											 {echo'<tr><td colspan="6">Carga Horario Docente, Pendiente Revisar Despues</td></tr>';}
											 $sqli_asignaciones->free();
										 }
									 }
									 else
									 { echo'<tr><td colspan="6">Sin Toma de Ramos Realizada</td></tr>';}
									 $sqli_tr->free();
									$conexion_mysqli->close();
									
									if(DEBUG){ var_dump($ARRAY_HORARIO); echo"<br><br>";}
									...
									for($d=0;$d<7;$d++)
									{
										if(isset($ARRAY_HORARIO[$d])){$aux_array_datos=$ARRAY_HORARIO[$d]; $hay_datos=true;}
										else{$aux_array_datos=array(); $hay_datos=false;}
										asort($aux_array_datos);
										//var_dump($ARRAY_HORARIO);
										
										if($hay_datos)
										{
											if(DEBUG){ var_dump($aux_array_datos);}
											
											foreach($aux_array_datos as $n => $array_info)
											{
												$aux_hora_inicio=$array_info["hora_inicio"];
												$aux_hora_fin=$array_info["hora_fin"];
												$aux_sala=$array_info["sala"];
												$aux_id_funcionario=$array_info["docente"];
												$aux_cod_asignatura=$array_info["cod_asignatura"];
												list($nombre_asignatura, $nivel_asignatura)=NOMBRE_ASIGNACION($id_carrera, $aux_cod_asignatura);
												$nombre_profesor=NOMBRE_PERSONAL($aux_id_funcionario);
												
												echo'<tr>
														<td>'.$array_dia[$d].'</td>
														<td>'.$aux_hora_inicio.'</td>
														<td>'.$aux_hora_fin.'</td>
									...
									?>
                                    <tr>
                                    	<td colspan="6"><em>*Importante: Este Horario se genera Automaticamente en Base a su Toma de Ramos y de la Carga Horario Docente, si Existen diferencias Favor Comunicar con Direccion Academica.</em></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div>
