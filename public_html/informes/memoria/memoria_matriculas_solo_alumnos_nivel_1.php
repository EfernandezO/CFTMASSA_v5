...
//--------------CLASS_okalis------------------//
	...
	$O->clave_del_archivo=md5("memoria_alumnos_nivel_1");
...
<title>Memoria Matriculas alumnos Nivel 1</title>
...
<body>
<h1 id="banner">Administrador - Memoria Matriculas Alumnos Nivel 1 CFT</h1>
...
<div id="apDiv1">
   ...
   else{ $condicion_sede="AND alumno.sede='$sede'";}
   
   	$aux=0;	 
	$verificar_contrato=true;
	$no_mostrar_retirados=true;
	$year_actual=date("Y");
	$cons_main_1="SELECT DISTINCT (id_alumno) FROM (contratos2 INNER JOIN alumno ON contratos2.id_alumno = alumno.id) INNER JOIN carrera ON alumno.id_carrera = carrera.id WHERE contratos2.nivel_alumno='1' $condicion_sede ORDER by alumno.id_carrera";
		...
		if($num_reg_M>0)
		{
			...
			while($DID=$sqli_main_1->fetch_row())
			{
				...
				$cons="SELECT DISTINCT(ano) FROM contratos2 WHERE contratos2.id_alumno='$id_alumno' AND contratos2.condicion<>'inactivo' AND contratos2.nivel_alumno='1' ORDER by contratos2.id DESC";
  ...
  <thead>
    <tr>
      <th colspan="100">Memoria Matriculas nivel 1 CFT(alumnos Matriculados <?php echo $sede;?> x a&ntilde;o contrato no considera situacion)</th>
    </tr>
    </thead>
    <tbody>
    <?php
...
<div id="apDiv2"><a href="memoria_matriculas_solo_alumnos_nivel_1.php?sede=Talca" class="button_R">Talca</a> <a href="memoria_matriculas_solo_alumnos_nivel_1.php?sede=Linares" class="button_R">Linares</a> <a href="memoria_matriculas_solo_alumnos_nivel_1.php" class="button_R">Todas</a></div>
</body>
</html>