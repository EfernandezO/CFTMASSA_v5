...
////////////////////
		...
		if($S_semestre==1){ $semestre_label="Primer Semestre";}
		else{$semestre_label="Segundo Semestre";}
		
		if($S_year<$year_actual){ $verbo="fue";}
		elseif($S_year==$year_actual)
			{
				if($S_semestre<$semestre_actual){ $verbo="fue";}
				else{ $verbo="es";}
			}
		else{ $verbo="ser�";}
		...
		//------------------------------------------------------------//
		$cons_N="SELECT nivel_alumno, nivel_alumno_2, vigencia FROM contratos2 WHERE id_alumno='$id_alumno' AND id_carrera='$id_carrera' AND yearIngresoCarrera='$yearIngreso' AND ano='$S_year' ORDER by id DESC LIMIT 1";
		$sqli_N=$conexion_mysqli->query($cons_N)or die($conexion_mysqli->error);
		$num_registros=$sqli_N->num_rows;
			$DCN=$sqli_N->fetch_assoc();
			$DCN_vigencia=$DCN["vigencia"];
			$DCN_nivel_alumno_al_realizar_contrato=$DCN["nivel_alumno"];
			$DCN_nivel_alumno_al_realizar_contrato_2=$DCN["nivel_alumno_2"];
		$sqli_N->free();	
		if(DEBUG){ echo"---->$cons_N<br>vigencia: $DCN_vigencia<br>nivel alumno al realizar el contrato: (semestre 1) $DCN_nivel_alumno_al_realizar_contrato (semestre 2) $DCN_nivel_alumno_al_realizar_contrato_2<br>N. registros: $num_registros<br>";}
		
		
		$DCN_nivel_alumno_al_realizar_contrato_final="";
		switch($DCN_vigencia)
		{
			case"semestral":
				$DCN_nivel_alumno_al_realizar_contrato_final=$DCN_nivel_alumno_al_realizar_contrato;
				break;
			case"anual":	
				if($S_semestre==1)
				{$DCN_nivel_alumno_al_realizar_contrato_final=$DCN_nivel_alumno_al_realizar_contrato;}
				else
				{$DCN_nivel_alumno_al_realizar_contrato_final=$DCN_nivel_alumno_al_realizar_contrato_2;}
				break;
		}
		
		//$DCN_nivel_alumno_al_realizar_contrato_final=4;
	...
	$parrafo_4='Se extiende el presente certificado a solicitud del(la) interesado(a) para ser presentado a '.utf8_decode($S_observacion).".";
	...
	//---------INICIO ESCRITURA---------//
		...
		$infoQr="Certificado Alumno Regular de CFT Massashusetts Sede: $sede_alumno Fecha Generacion: $fecha_actual Rut: Alumno: $rut_alumno Carrera Alumno: $carrera_alumno Periodo $semestre_actual - $year_actual Codigo. $CODIGO_GENERACION URL VALIDACION: https://intranet.cftmassachusetts.cl/serviciosExternos/validarCertificado.php?codigo=".base64_encode($CODIGO_GENERACION)."&id_solicitud=".base64_encode($id_solicitud);
		$imgQR='https://intranet.cftmassachusetts.cl/libreria_publica/phpqrcode/ImagenQRv2.php?qr_info='.base64_encode($infoQr);
		if($verQr){$pdf->image($imgQR,14,210,50,50,'png');}
		...
		$pdf->Text(14,265,"*Cod.".$CODIGO_GENERACION."*");
