...
if($hay_solicitud)
{
	...
	$sql_c->free();	
		...
		{
			$action="";
			$url="alumno_regular_v1_2.php?id_solicitud=$id_solicitud";
			if(DEBUG){ echo"URL: $url";}
			else{ header("location: $url");}
		}
		else
		{$action="alumno_regular_v1_2.php";}
}
else
...
<label for="firmas" class="sr-only">Firma</label>
  ...
  <input type="text" id="presentado_a" name="presentado" class="form-control" placeholder="telefono contacto" value="<?php echo $A_fono;?>" required>
            <?php }?>
