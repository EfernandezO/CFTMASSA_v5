...
if($continuar){
	...
	$cons="SELECT * FROM bolsaTrabajo WHERE id='$id' LIMIT 1";
	$sqli=$conexion_mysqli->query($cons)or die($conexion_mysqli->error);
	$BT=$sqli->fetch_assoc();
		$auxTitulo=$BT["titulo"];
		$auxFechaGeneracion=$BT["fechaGeneracion"];
		$auxCuerpo=$BT["cuerpo"];
	$sqli->free();
...
<body>
<h1 id="banner">Edicion - Oferta </h1>
  ...
  <form action="<?php echo $action;?>" method="post" enctype="multipart/form-data" name="frm" id="frm">
    <div align="center">
      <table width="60%" height="257" border="0" align="center">
        <thead>
          <tr>
            <th colspan="2"><div align="center" class="Estilo3">Edicion de Noticia </div></th>
          </tr>
        </thead>
        <tbody>
          <tr class="odd">
            ...
            <td><label>
              <input name="ftitulo" type="text" id="ftitulo" value="<?php echo $auxTitulo;?>" />
              <input name="idOferta" type="hidden" id="idOferta" value="<?php echo $id;?>" />
            </label></td>
          </tr>
          <tr class="odd">
            <td valign="top"><span class="Estilo3">Oferta</span></td>
            <td><label>
              <textarea name="txt_noticia" id="txt_noticia"><?php echo $auxCuerpo;?></textarea>
              </label>
                <label><br />
              </label></td>
          </tr>
