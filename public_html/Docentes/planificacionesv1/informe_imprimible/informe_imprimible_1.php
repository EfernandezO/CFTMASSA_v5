...
//-------------------------------------------------------------------------------------------------------//	
	...
	$cons_NH="SELECT SUM(horas_semana) FROM planificaciones WHERE id_carrera='$id_carrera' AND sede='$sede' AND cod_asignatura='$cod_asignatura' AND semestre='$semestre' AND year='$year' AND id_funcionario='$id_usuario' AND jornada='$jornada' AND grupo='$grupo_curso'";
	...
	//max numero semana
	$cons_NS="SELECT COUNT(DISTINCT(numero_semana)) FROM planificaciones WHERE id_funcionario='$id_usuario' AND id_carrera='$id_carrera' AND cod_asignatura='$cod_asignatura' AND semestre='$semestre' AND year='$year' AND sede='$sede' AND jornada='$jornada' AND grupo='$grupo_curso'";
...
<div id="apDiv3">
  ...
  <?php if($condicion_horas and $condicion_semanas){ echo"Sin Errores, Puede Imprimir"; echo'<br><br><a href="informe_imprimible_2.php?id_funcionario='.base64_encode($id_usuario).'&sede='.base64_encode($sede).'&id_carrera='.base64_encode($id_carrera).'&jornada='.base64_encode($jornada).'&grupo='.base64_encode($grupo_curso).'&asignatura='.base64_encode($cod_asignatura).'&semestre='.base64_encode($semestre).'&year='.base64_encode($year).'" class="button_R" target="_blank">Click para Imprimir</a>';}else{ echo"Existen Errores, Por favor corregir para poder Imprimir";}?></span>
  <br><br><br>
  <a href="../../contenidos/informe_imprimible/exportar_planificacion_a_xlsx.php?id_funcionario=<?php echo base64_encode($id_usuario);?>&amp;sede=<?php echo base64_encode($sede);?>&amp;id_carrera=<?php echo base64_encode($id_carrera);?>&amp;jornada=<?php echo base64_encode($jornada);?>&amp;grupo=<?php echo base64_encode($grupo_curso);?>&amp;asignatura=<?php echo base64_encode($cod_asignatura);?>&amp;semestre=<?php echo base64_encode($semestre);?>&amp;year=<?php echo base64_encode($year);?>" class="button_R" target="_blank">Click para XLSX</a>
  </div>
</body>
</html>