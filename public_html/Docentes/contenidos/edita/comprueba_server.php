...
function COMPRUEBA($num_semana, $id_contenido, $id_contenidoMain)
{
	...
	$sqliPM->free();	
	//cantidad de horas de duracion de la clase, que actualemnte tiene registrada
	$cons_DC="SELECT duracion_clase FROM contenidosDetalle WHERE idContenidoMain='$id_contenidoMain' AND id_contenido='$id_contenido'";
	$sqli_DC=$conexion_mysqli->query($cons_DC)or die($conexion_mysqli->error);
		$DC=$sqli_DC->fetch_assoc();
		$duracion_clase_actual=$DC["duracion_clase"];
	$sqli_DC->free();
		//------------------------------------------------------------------//
			...
			$sqli_HT->free();
			//----------------------------------------------------//
			$numero_horas_X_semana=($TOTAL_HORAS_PROGRAMA/$numeroSemanas);
			//---------------------------------------------------//
			//$objResponse->Alert("N horas X semana: $numero_horas_X_semana<br>");
			
		//cuantas horas van en esta semana sin contar la planificacion actual 03/10/2017
		$cons_HS="SELECT SUM(duracion_clase) FROM contenidosDetalle WHERE idContenidoMain='$id_contenidoMain' AND id_contenido<>'$id_contenido' AND numero_semana='$num_semana'";
		...
		//$objResponse->Alert("N horas planificadas en semana [$num_semana]: $num_horas_planificadas_en_semana");
		//------------------------------------------------------------//
