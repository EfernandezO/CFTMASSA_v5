...
//-------------------------------------------------------------------------------------------------------//	
	...
	$cons_NH="SELECT SUM(duracion_clase) FROM contenidosDetalle WHERE idContenidoMain='$id_contenidoMain'";
	...
	//max numero semana
	$cons_NS="SELECT COUNT(DISTINCT(numero_semana)) FROM contenidosDetalle WHERE idContenidoMain='$id_contenidoMain'";
...
<title>Imprime Contenidos</title>
<style type="text/css">
#apDiv1 {
	position:absolute;
	width:90%;
...
<div id="apDiv3">
  ...
  <?php if($condicion_horas and $condicion_semanas){ echo"Sin Errores, Puede Imprimir"; echo'<br><br><a href="informe_imprimible_2.php?id_contenidoMain='.base64_encode($id_contenidoMain).'" class="button_R" target="_blank">Click para Imprimir</a>';}else{ echo"Existen Errores, Por favor corregir para poder Imprimir";}?></span>
  <br><br><br>
  <a href="exportar_contenido_a_xlsx.php?id_contenidoMain=<?php echo base64_encode($id_contenidoMain);?>" class="button_R" target="_blank">Click para XLSX</a>
  </div>
</body>
</html>