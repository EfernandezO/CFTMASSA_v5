...
function COMPRUEBA($num_semana, $id_contenidoMain)
{
	...
	$div='div_info';
		...
		$cons_HS="SELECT SUM(duracion_clase) FROM contenidosDetalle WHERE idContenidoMain='$id_contenidoMain' AND numero_semana='$num_semana'";
		...
		if(DEBUG){$objResponse->Alert("N horas usadas en semana [$num_semana]: $num_horas_planificadas_en_semana");}
		//------------------------------------------------------------//
		...
		for($y=1;$y<=$horas_faltantes_para_semana;$y++)
		{
