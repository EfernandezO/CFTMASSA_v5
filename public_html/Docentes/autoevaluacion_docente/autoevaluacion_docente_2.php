...
//-----------------------------------------//
  ...
  if($continuar)
  {
	  ...
	  if($id_encuesta>0)
	  {
		  $cons_RE="SELECT COUNT(id_resultados) FROM encuestas_resultados WHERE id_encuesta='$id_encuesta' AND id_usuario_evaluar='$id_docente' AND semestre_evaluar='$semestre_periodo' AND year_evaluar='$year_periodo'";
	   ...
	   if($num_preguntas>0)
	   {
		   $contador=0;
			while($M=$sql->fetch_assoc())
			{
				...
				{
					$action="autoevaluacion_docente_3.php";
...
<title>Autovaluacion Docente</title>
...
<input name="id_usuario" type="hidden" value="<?php echo $id_docente; ?>" />
