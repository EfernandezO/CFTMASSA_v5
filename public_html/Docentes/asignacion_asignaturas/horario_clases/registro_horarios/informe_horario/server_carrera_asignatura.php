...
function GENERA_INFORME($FORMULARIO)
{
	...
	if($num_registros>0)
	{
		...
		while($TM=$sqli->fetch_assoc())
		{
			...
			//-----------------------------------------------------//
			if($situacion_alumno_en_periodo=="V")
					...
					foreach($array_4 as $aux_grupo => $array_5)
					{
						...
						foreach($array_5 as $aux_cod_asignatura => $valor)
						{
							...
							$tabla.='<tr>
										...
										<td align="center"><a href="informe_horario_2.php?sede='.base64_encode($aux_sede).'&id_carrera='.base64_encode($aux_id_carrera).'&nivel='.base64_encode($aux_nivel).'&jornada='.base64_encode($aux_jornada).'&grupo='.base64_encode($aux_grupo).'&cod_asignatura='.base64_encode($aux_cod_asignatura).'&semestre='.base64_encode($semestre).'&year='.base64_encode($year).'" target="_blank" title="Revisar asistencia" class="button_R">Revisar</a></td>
									</tr>';
						}
